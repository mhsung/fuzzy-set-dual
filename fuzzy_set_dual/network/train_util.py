...
def evaluate_positive_and_negative_pairs(sess, net, data):
    count = 0
    pos_accu = 0.0
    neg_accu = 0.0

    for X, Y in data:
        step_pos_accu, step_neg_accu = sess.run([
            net.pos_accu, net.neg_accu], feed_dict={
                net.X: X,
                net.Y: Y,
                net.is_training: False})

        count += data.step_size
        pos_accu += (step_pos_accu * data.step_size)
        neg_accu += (step_neg_accu * data.step_size)

    pos_accu /= float(count)
    neg_accu /= float(count)
    return pos_accu, neg_accu
...
def train(sess, net, train_data, val_data, n_epochs, snapshot_epoch,
    ...
    for epoch in range(1, n_epochs + 1):
        for X, Y in train_data:
            step, _, _ = sess.run([net.global_step, net.train_op,
                net.clip_tol_op], feed_dict={
                net.X: X,
                net.Y: Y,
